...
class OrderClass
{
    ...
    function getAllOrders($conn,$date)
    {
      ...
      function getOrderByStatus($conn,$status,$date)     
      {
       ...
       function getOrderByStatusPlaced($conn,$status1,$status2,$date)     
      {
          $query  = "select o.*,od.amount_range,s.code_description from orders o join order_details od on o.order_id = od.order_id join statuscode s on s.code_id = o.order_status where o.order_status != '$status1' and o.order_status!='$status2' and o.order_date = '$date'";
        $result = mysqli_query($conn, $query);
        $count  = mysqli_num_rows($result);
